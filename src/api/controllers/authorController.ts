...
const getAuthors = (req: Request, res: Response<Author[]>) => {
  const authors = getAllAuthors();
  res.json(authors);
};

const getAuthorByID = (req: Request<{id: string}>, res: Response<Author>) => {
  try {
...
const authorDelete = (
  ...
  try {
    ...
    next(new CustomError((error as Error).message, 204));
  }
};

export {getAuthors, getAuthorByID, authorPut, authorPost, authorDelete};
